 ...
 * @return array
 */
function soma_body_classes( $classes ) {
...
add_filter( 'body_class', 'soma_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function soma_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'soma_pingback_header' );
 ...
 * @return array
 */
function soma_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'soma_filter_script_loader_tag', 10, 2 );
 ...
 */
function soma_get_preload_stylesheet_uri( $wp_styles, $handle ) {
...
function soma_add_body_style() {
	...
	// Preload content.css.
	$preloads['soma-content'] = soma_get_preload_stylesheet_uri( $wp_styles, 'soma-content' );

	// Preload sidebar.css and widget.css.
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		$preloads['soma-sidebar'] = soma_get_preload_stylesheet_uri( $wp_styles, 'soma-sidebar' );
		$preloads['soma-widgets'] = soma_get_preload_stylesheet_uri( $wp_styles, 'soma-widgets' );
	}

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['soma-comments'] = soma_get_preload_stylesheet_uri( $wp_styles, 'soma-comments' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['soma-front-page'] = soma_get_preload_stylesheet_uri( $wp_styles, 'soma-front-page' );
	}
...
add_action( 'wp_head', 'soma_add_body_style' );
 ...
 */
function soma_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'soma_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function soma_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'soma_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'soma_add_nav_menu_aria_current', 10, 2 );
